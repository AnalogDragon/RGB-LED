...
void SetSysClock(void){
    RCC_DeInit();
    RCC_HSICmd(ENABLE);//8M
//    RCC_HSI14Cmd(ENABLE);//8M
    while(RCC_GetFlagStatus(RCC_FLAG_HSIRDY) == RESET);//等待HSI使能成功
    FLASH_PrefetchBufferCmd(ENABLE);
    FLASH_SetLatency(FLASH_Latency_1);
    RCC_HCLKConfig(RCC_SYSCLK_Div1); //AHB 时钟 8M
    RCC_PCLKConfig(RCC_HCLK_Div1); //设置APB时钟 8M
    RCC_SYSCLKConfig(RCC_SYSCLKSource_HSI);//设置HSI为系统时钟
    while(RCC_GetSYSCLKSource() != 0x00);//判断HSI是否为系统时钟
}
